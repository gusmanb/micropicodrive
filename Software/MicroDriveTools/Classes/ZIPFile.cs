namespace Kyrathasoft.FilesAndDirectories.ZipStorer
{
    ...
    public class ZipStorer : IDisposable
    {
        ...
        public List<ZipFileEntry> ReadCentralDir()
        {
            ...
            for (int pointer = 0; pointer < this.CentralDirImage.Length;)
            {
                ...
                if (extraSize != 0)
                    zfe.ExtraField = CentralDirImage.Skip(pointer + 46 + filenameSize).Take(extraSize).ToArray();
        ...
        */
        private void WriteLocalHeader(ref ZipFileEntry _zfe)
        {
            ...
            ushort extraLen = (ushort)(_zfe.ExtraField == null ? 0 : _zfe.ExtraField.Length);

            this.ZipFileStream.Write(BitConverter.GetBytes(extraLen), 0, 2); // extra length

            this.ZipFileStream.Write(encodedFilename, 0, encodedFilename.Length);

            if (_zfe.ExtraField != null && _zfe.ExtraField.Length > 0)
                this.ZipFileStream.Write(_zfe.ExtraField, 0, _zfe.ExtraField.Length);
